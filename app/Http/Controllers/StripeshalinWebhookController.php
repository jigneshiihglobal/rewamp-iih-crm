...
class StripeshalinWebhookController extends Controller
{
    public $service;
    ...
    public function handleWebhook(Request $request)
    {
        ...
        Stripe::setApiKey(config('stripe.shalin_secret_key'));
        $endpointSecret = config('stripe.shalin_webhook_key');
        ...
        try {
            ...
            if(isset($invoices) && !empty($invoices) && $invoices->count() > 0){
                if ($invoices->count() === 1) {
                    ...
                    if($invoice->client->workspace->slug == 'shalin-designs'){
    ...
    public function receivedPaymentStore($invoice,$amount){
            ...
            $payment_source = PaymentSource::where('title','Stripe')->where('workspace_id',2)->first();
            DB::beginTransaction();
        try{
            ...
            $workspace_id = $invoice->client->workspace->id ?? 2;
