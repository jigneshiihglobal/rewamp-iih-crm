            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header p-1">
                        <h4 class="card-title">Create Marketing Expenses</h4>
                    </div>
                    <hr class="m-0">
                    <div class="card-body p-75">
                        <form class="form repeater" id="createExpensesForm" action="{{ route('marketing.expenses.store-many') }}" method="POST">
                            @csrf
                            @if (Route::is('marketing.expenses.edit'))
                                @method('PUT')
                            @endif
                            <div id="expenses">
                                            ...
                                            <div class="row mx-0">
                                                <div class="col-xl-2 col-lg-3 col-md-4 col-sm-4 col-12 px-50">
                                                    <div class="mb-1">
                                                        ...
                                                        <input type="text" class="form-control form-control-sm flatpickr"
                                                            placeholder="Expense Date" name="marketing_expense_date"
                                                            data-name="marketing_expense_date" value="{{ date('d/m/Y') }}"
                                                ...
                                                <div class="col-xl-2 col-lg-3 col-md-3 col-sm-4 col-12 px-50">
                                                    <div id="select-container">
                                                        ...
                                                        <div class="mb-1">
                                                            <label class="form-label">Marketing Expense Type <span class="text-danger">*</span></label>
                                                            <div class="position-relative">
                                                                <select class="select2-size-sm form-select select2 expense_type_select2" name="marketing_expense_type_id[]"
                                                                        data-name="marketing_expense_type_id" data-rule-required="true"
                                                                        data-msg-required="Please select expense type" data-allow-clear="true" data-create-new-option="true">
                                                                    @if(isset($expense_types) && !empty($expense_types))
    ...
    <script>
        $(document).ready(function() {
            ...
            function setDefaults($expenseItem, defaults = {}) {
                $newItem.find("[name=marketing_expense_date]").val(defaults?.marketing_expense_date);
                $newItem.find("[name=marketing_expense_type_id]").val(defaults?.marketing_expense_type_id);
            ...
            function createExpense(defaults = {}) {
                ...
                $newItem.on('change', `[name="expenses[${maxIndex}][marketing_expense_type_id]"]`, '')

                initSelect2($newItem);
                initExpSelect2($newItem);
            ...
            function initExpSelect2($parent) {
                $parent.find('select.expense_type_select2').each(function(select2Index) {
                    let $select = $(this);
                    let exp_config = {
                        width: "100%",
            ...
            function removeBtnClickHandler(e) {
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $item.on(
                            'change',
                            `[name="expenses[${maxIndex}][marketing_expense_type_id]"]`,
            ...
            function copyPreviousData($item) {
                ...
                if($prevItem.find('[data-name=marketing_expense_date]').val()) {
                    let fp = $item.find('[data-name=marketing_expense_date]').get(0)._flatpickr;
                    let prevFp = $prevItem.find('[data-name=marketing_expense_date]').get(0)._flatpickr;
