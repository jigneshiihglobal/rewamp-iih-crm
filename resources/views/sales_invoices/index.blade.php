...
<div class="row align-items-start px-0 mb-1" id="PaymentFilters">
    ...
    <select id="is_sales_invoice_status" class="form-select form-select-sm select2">
        ...
        <option value="4">Mail Send</option>
...
<script>
    $(document).ready(function () {
        ...
        if (allowedRoutes.includes(currentRoute) && !allowedRoutes.includes(previousRoute)) {
            sessionStorage.removeItem('user_sales_invoice_status_filter');
        }

        const savedStatus = sessionStorage.getItem('user_sales_invoice_status_filter');
        if (savedStatus !== null) {
        ...
        const table = $('.user-invoices-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('sales_invoices.index') }}",
                data: function (d) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    name: 'status',
                    render: function (data, type, full) {
                        let status = data?.toLowerCase?.() || '1';

                        switch (status) {
                            case '1':
                                return '<span class="badge rounded-pill custom-badge-pending custom-badge-line">Pending</span>';
                            case '4':
                                return '<span class="badge bg-gradient bg-info rounded-pill custom-badge-line">Mail Send</span>';
                            case '2':
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                            ...
                            // Statuses where Edit/Delete are not allowed
                            const restrictedStatuses = ['2', '3','4'];
                            // Tooltip messages for status
                            const statusMessages = {
                                '2': 'Action not allowed for approved invoice.',
                                '3': 'Action not allowed for rejected invoice.',
                                '4': 'Action not allowed Invoice already sent to admin.'
                            };
                        ...
                        return `
                            <a class="btn btn-sm btn-icon btn-flat-secondary" 
                                title="Detail" 
                                href="${route('sales_invoices.show', { user_invoice: full.encrypted_id })}" 
                                data-bs-toggle="tooltip">
                                    ${feather.icons['eye'].toSvg({ class: 'font-medium-3' })}
                            </a>
                            <a class="btn btn-sm btn-icon btn-flat-info ${isRestricted ? 'restricted-btn' : ''}" 
                                title="${isRestricted ? tooltipMessage : 'Edit'}"
                                ${isRestricted ? `data-restrict-msg="${tooltipMessage}" href="javascript:void(0);"` : `href="${route('sales_invoices.edit', { user_invoice: full.encrypted_id })}"`}
                                data-bs-toggle="tooltip">
            ...
            dom: '<"d-flex justify-content-between align-items-center header-actions mx-1 row mt-50 user-invoices-table-wrapper"r' +
        ...
        $('.user-invoices-table').on('draw.dt', function () {
        ...
        $('.user-invoices-table').on('draw.dt', function () {
         ...
         $('#is_sales_invoice_status').on('change', function () {
            ...
            sessionStorage.setItem('user_sales_invoice_status_filter', selectedStatus);
            table.ajax.reload();
        });
        ...
        $(document).on('click', '.payment_delete', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: route('sales_invoices.destroy', id),
                        method: 'DELETE',
                        data: {
