                        ...
                        <div class="d-flex justify-content-between flex-md-row flex-column mt-0">
                            ...
                            <div class="mt-md-0 mt-2 d-flex flex-column invoice-spacing align-items-md-end">
                                ...
                                <div class="payment-buttons  mt-auto">
                                    <a href="{{ route('invoices.edit', $invoice->encrypted_id) }}"
                                    ...
                                    @endif
                                    <a href="#" class="btn btn-sm btn-outline-warning waves-effect" id="mailsend">
                                        <span>Send Mail</span>
                                        <i data-feather="send"></i>
                                    </a>

                                    @if ($invoice->payment_status == 'paid')
                                        <a class="btn btn-sm btn-outline-success waves-effect" id="paymentLinkBtn">
                                    ...
                                    @else
                                        ...
                                        @if($invoice->payment_reminder == 1)
                                            <a class="btn btn-sm btn-outline-success waves-effect payment_reminder_cls" id="payment_reminder_btn" title="Payment reminder enable" data-remindervalue="{{$invoice->payment_reminder}}" data-invoiceid="{{$invoice->id}}" style="padding: 0.2rem 1.2rem;">
                                                <i data-feather="check-circle"></i>
                                            </a>
                                        @else
                                            <a class="btn btn-sm btn-outline-soundcloud payment_reminder_cls" id="payment_reminder_btn" title="Payment reminder disable"  data-remindervalue="{{$invoice->payment_reminder}}" data-invoiceid="{{$invoice->id}}" style="padding: 0.2rem 1.2rem;">
                                                <i data-feather="slash"></i>
                                            </a>
                                        @endif
                                    @endif
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.attach_file', function(e){
                ...
                for (let i = 0; i < fileContentsArray.length; i++) {
                    if (fileContentsArray[i].name === attach_name) {
                        $("[id*='attachment_data_" + attach_name + "']").remove();
                        fileContentsArray.splice(i, 1);
                        break;
                    }
                }
            });
            ...
            var sendReceiptMailForm = $('#sendReceiptMailForm').validate({
                ...
                submitHandler: function (form, event) {
                    ...
                    /*const custom_attach = fileContentsArray.length;
                      var custom_file = fileContentsArray;

                      if (custom_file.length > 0) {
                          for (let i = 0; i < custom_file.length; i++) {
                              const file = custom_file[i];
                              formData.append('files[]', file);
                          }
                      }*/

                    /*custom_file.forEach((file, index) => {
